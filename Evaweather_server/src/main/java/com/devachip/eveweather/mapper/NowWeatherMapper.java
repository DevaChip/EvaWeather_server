...
public class NowWeatherMapper {
	private String selectData = 
			...
			+ "FROM " + UltraSrtFcst.tableName + " usf "
			+ "LEFT JOIN " + UltraSrtNcst.tableName + " usn "
			...
			+ "	FROM (SELECT TMN, fcstDate , nx, ny FROM " + VilageFcst.tableName + " WHERE fcstTime = '0600') tmn "
			+ "	LEFT JOIN (SELECT TMX, fcstDate, fcstTime, nx, ny FROM " + VilageFcst.tableName + " WHERE fcstTime = '1500') tmx "
			+ "		ON tmn.fcstDate = tmx.fcstDate "
			+ "		AND tmn.nx = tmx.nx "
			+ "		AND tmn.ny = tmx.ny "
			+ "	LEFT JOIN (SELECT POP, fcstDate, fcstTime, nx, ny FROM " + VilageFcst.tableName + " WHERE fcstTime = ?) pop "
